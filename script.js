class SnakeGame {
    ...
    init() {
        ...
        this.tailSize = 5
        this.trail = []
        this.gridSize = this.titleCount = 20
        this.VelocityX = this.VelocityY = 0

        this.timer = setInterval(this.loop.bind(this), 1000 / 8)
    ...
    update() {
        ...
        if (this.positionX < 0) {
            this.positionX = this.titleCount - 1
        }
        if (this.positionY < 0) {
            this.positionY = this.titleCount - 1
        }
        if (this.positionX > this.titleCount - 1) {
            this.positionX = 0

        }
        if (this.positionY > this.titleCount - 1) {
            this.positionY = 0
        }
        ...
        if (this.appleX === this.positionX && this.appleY === this.positionY) {
            this.tailSize++
            this.appleX = Math.floor(Math.random() * this.titleCount)
            this.appleY = Math.floor(Math.random() * this.titleCount)
        }
    }
draw() {
